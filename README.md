...
附带了一个零依赖 Node.js 后端，用来记录"谁在访问网站"。
...
后台可看：访问时间、IP、设备类型（PC/触屏）、浏览器型号/系统、访问页面、屏幕分辨率、停留时长、登录会员名。数据保存在 `visits.json`（最多 5000 条）。

不启动后端也能用：直接双击 `index.html` 打开即可，所有前端功能正常，只是没有访问日志。
